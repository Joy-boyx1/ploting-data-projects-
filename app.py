...
if uploaded_file:
    try:
        ...
        if st.session_state.show_etat:
            ...
            sns.barplot(data=counts, x=col_etat, y='Nombre', palette="Oranges_r", ax=ax1)
            plt.xticks(rotation=45)
            st.pyplot(fig1)
            st.dataframe(counts, use_container_width=True)

        # 2. BLOC ECART BUDGÉTAIRE (AVEC CALCUL % D'ÉCART)
        if st.session_state.show_budget:
            st.write("---")
            st.write("### 💸 Comparaison Budget vs Adjugé (Par Affaire)")
            
            # Filtrage strict (élimine les affaires incomplètes)
            ...
            selected_site = st.selectbox("📍 Filtrer par Site (Ecart) :", options=["Tous les sites"] + sites)
            ...
            if not df_plot.empty:
                # CALCUL DU POURCENTAGE D'ÉCART : (Adjugé / Budget) * 100
                df_plot['% d’Écart'] = ((df_plot[col_adjuge] / df_plot[col_budget]) * 100).round(2)
                
                # On limite à 15 pour le graph
                df_view = df_plot.head(15)
                df_melt = df_view.melt(id_vars=[col_titre], value_vars=[col_budget, col_adjuge], var_name='Type', value_name='Montant')
                ...
                # Tableau avec la nouvelle colonne
        ...
        if st.session_state.show_site_analysis:
            ...
            if not df_site_group.empty:
                # CALCUL % CONSOMMATION GLOBALE PAR SITE
