...
if uploaded_file_aznag:
    try:
        ...
        # Définition des colonnes (A=0, C=2, G=6, J=9, N=13)
        col_exercice = df_aznag.columns[0]
        col_sites = df_aznag.columns[2]    # Colonne C
        col_etat = "Etat"
        ...
        # --- FILTRE GÉNÉRAL : EXERCICE ---
        exercices_disponibles = sorted(df_aznag[col_exercice].dropna().unique().astype(str), reverse=True)
        selected_year = st.selectbox("📅 Sélectionner l'EXERCICE :", options=["Tous"] + exercices_disponibles)
        ...
        # 4. ANALYSE ECART BUDGÉTAIRE + SOUS-FILTRE SITES
        if st.session_state.show_budget:
            st.write("---")
            st.write("### 💰 Analyse Budgétaire")
            
            # --- SOUS-FILTRE SITES (Uniquement ici) ---
            sites_disponibles = sorted(df_filtered[col_sites].dropna().unique().astype(str))
            selected_site = st.selectbox("📍 Filtrer par Site (Colonne C) :", options=["Tous les sites"] + sites_disponibles)
            
            df_budget_plot = df_filtered.copy()
            if selected_site != "Tous les sites":
                df_budget_plot = df_budget_plot[df_budget_plot[col_sites].astype(str) == selected_site]

            # Filtrage des lignes avec montants significatifs
            df_plot_data = df_budget_plot[(df_budget_plot[col_budget] > 0) | (df_budget_plot[col_adjuge] > 0)].head(20)
            ...
            if not df_plot_data.empty:
                ...
                st.write(f"**Détails financiers - Site : {selected_site}**")
                st.dataframe(df_plot_data[[col_titre, col_sites, col_budget, col_adjuge, 'Écart']], use_container_width=True)
            else:
                st.warning(f"Aucune donnée budgétaire pour le site : {selected_site}")
